...
```text
1\. 조사 이미지 등록(여러 장, 선택 즉시 업로드)
2\. 분석 시작 접수 및 상태 폴링
3\. 완료되면 보고서 자동 조회 - 특이점 위치 오버레이 확인
4\. (도자기 재질이면) 도자기 보조 검사 자동 실행
5\. PDF 리포트 생성·다운로드
6\. 육안 조사 완료
```

- 조사 이미지는 여러 장 등록할 수 있으며, 선택 즉시 서버로 업로드/삭제가
  반영됩니다(로컬 임시 상태 없음).
- 분석 시작 시 vca_v2 8단계 파이프라인(`preprocessing →
  rough_masking → visual_cue_generation → rag → prompt_generating →
  mask_refining → anomaly_grouping → report_generating`)의 진행
  상태를 2초 간격으로 폴링해 단계별 체크리스트로 보여줍니다.
- 실패한 run은 직전과 같은 이미지 집합일 때 이어서 분석(`resume`)할
  수 있습니다.
- 완료되면 보고서를 자동으로 불러오고, 원본 사진 위에 특이점 위치를
  번호 배지 + 폴리곤 오버레이로 표시합니다. 사진은 확대/축소와
  드래그 이동을 지원하며, 번호를 클릭하면 근거 문헌 인용까지 포함한
  상세 페이지로 이동합니다.
- 재질이 도자기 계열(백자·청자 등 포함)이면 완전성·유약·시대·문양
  검사가 서버에서 자동으로 이어서 실행됩니다 - 별도 버튼 클릭이 필요
  없고, 실패해도 VCA 보고서 자체는 그대로 보이며 수동 재시도할 수
  있습니다. 이 단계도 상태 체크리스트에 "도자기 특화 분석" 그룹으로
  같이 표시됩니다.
- `PDF 생성`으로 리포트 PDF를 만들고, 완료되면 새 탭에서 다운로드할
  수 있습니다.
- `육안 조사 완료` 클릭 시 `VISUAL` 상태를 `DONE`으로 바꾸고 유물
  워크스페이스로 돌아갑니다.

주의: 특이점을 사람이 직접 추가/삭제하거나 AI 판정(유형·심각도)을
수정하는 기능은 아직 없습니다 - 지금은 AI 결과를 검토만 할 수
있습니다. 도자기 문양 인식은 서버에 `OPENAI_API_KEY`가 설정돼 있어야
동작하며, 없으면 시대 판정(로컬 모델) 등 나머지는 정상이어도 문양
오버레이만 비어 있습니다.
...
### 4.6 AWS 배포 시 VCA(육안 조사) 환경변수

위 4.3의 표에는 없지만, 육안 조사(VCA) 기능이 실제로 쓰는 값들입니다.
AWS(EKS + CloudFront)에 배포할 때 CodeBuild 환경변수 또는 배포용
`.env`에 아래 값을 채워야 합니다.

| 환경변수 | 역할 |
| --- | --- |
| `VITE_API_BASE_URL` | BE(EKS Ingress/ALB) 공개 주소. VCA API(`/api/vca/**`)도 이 주소를 기준으로 호출합니다. |
| `VITE_VCA_ACCESS_TOKEN` | `/api/vca/**` 전체를 보호하는 게이트웨이 토큰을 `X-VCA-Access-Token` 헤더로 실어 보냅니다. BE `.env`/`app-secrets`의 `VCA_ACCESS_TOKEN`과 **정확히 같은 값**이어야 하며, 안 맞으면 이미지 업로드·분석 시작이 전부 실패합니다. |
| `VITE_USE_VCA_MOCK` | 운영 배포에서는 반드시 `false`. `true`면 실제 BE 호출 없이 화면 안 mock 데이터만 보여줍니다. |

BE 쪽 대응 설정(`VCA_AI_BASE_URL` 등 vca-ai 위치, 내부/외부 배포
모드)은 `cultural-heritage-be`의 README "VCA 분석 파트 README →
AWS 배포" 섹션을 참고하세요 - FE는 그쪽 구성과 무관하게 `VITE_API_BASE_URL` 하나만 BE 공개 주소로 맞추면 됩니다.
