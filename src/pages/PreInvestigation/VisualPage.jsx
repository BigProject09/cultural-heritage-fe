    ...
    <main className="visual-page visual-vca-page">
      <div className="visual-vca-container">
        <nav className="visual-breadcrumb" aria-label="현재 위치">
          <button type="button" onClick={() => navigate(getArtifactRoute(artifactId))}>
            유물 워크스페이스
          </button>
        ...
        <section className="visual-artifact-summary" aria-label="조사 대상 요약">
        ...
        <section className="photo-card visual-vca-layout-main" aria-labelledby="visual-images-title">
          <div className="photo-upload-row">
            <div>
              <h3 className="photo-upload-title" id="visual-images-title">조사 이미지</h3>
              <p className="status-note">원본 이미지는 변경되지 않으며, 업로드 후 분석 실행에 사용됩니다.</p>
            </div>
        ...
        <aside className="result-card visual-vca-layout-aside" aria-labelledby="visual-run-title">
          ...
          <p className="status-note">분석 상태는 실제 VCA API에서 주기적으로 갱신합니다. 진행 중인 분석은 언제든 중지할 수 있습니다.</p>
              ...
              {isPotteryMaterial(workspaceArtifact.material) && (
                <>
                  ...
                  {POTTERY_STAGE_ORDER.map((name) => {
                    const status = potteryStageStatus(report?.potteryInspectionStatus, working);
                    return (
                      <li key={name} className={`visual-vca-stage-item ${status}`}>
                        <span className="visual-vca-stage-name">{POTTERY_STAGE_LABELS[name]}</span>
                        <span className="visual-vca-stage-status">{STAGE_STATUS_LABELS[status]}</span>
                      </li>
                    );
                  })}
                </>
        ...
        {notice && <p className="visual-notice" role="status" aria-live="polite">{notice}</p>}
        {completionError && <p className="visual-notice visual-notice--error" role="alert">{completionError}</p>}

        {/* STEP 03: 조사 보고서 - main의 result-layout(result-card + review-card) 틀 재사용 */}
        <div className="visual-result-layout">
          <section className="result-card" aria-labelledby="visual-report-title">
            <div className="visual-section-heading">
              <div>
                <span>조사 보고서</span>
                <h2 id="visual-report-title">AI 분석 결과</h2>
              </div>
              {selectedRun && <span className="review-badge">{statusLabel(selectedRun.status)}</span>}
            </div>
            {report ? (
              <VisualReport
                ...
                artifactPeriod={workspaceArtifact.period}
          ...
          <aside className="visual-review-card">
            ...
            <h2>전문가 검수 안내</h2>
            <p>AI 결과는 조사 초안입니다. 실제 표면 상태와 촬영 조건을 함께 확인한 뒤 조사 결과를 확정하세요.</p>
            <ul>
        ...
        <footer className="complete-area">
          ...
          <button type="button" className="complete-btn" onClick={handleComplete} disabled={!canComplete}>
            육안 조사 완료
          </button>
        </footer>
      </div>
